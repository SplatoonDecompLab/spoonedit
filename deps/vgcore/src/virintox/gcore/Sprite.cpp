...
Graphics::Sprite::Sprite(const glm::vec2 &Pos, float scale, Texture* Tex) {
    ...
    std::vector<float> vertices{
            Pos.x                         , Pos.y,
            0.0f                      , 0.0f,
            Pos.x + TexPtr->getSize()->.x * scale, Pos.y,
            1.0f                      , 0.0f,
            Pos.x                         ,Pos.y + TexPtr->m_size.y * scale,
            0.0f                      , 1.0f,

            Pos.x + TexPtr->m_size.x * scale, Pos.y,
            1.0f                      , 0.0f,
            Pos.x + TexPtr->m_size.x * scale, Pos.y + TexPtr->m_size.y * scale,
            1.0f                      , 1.0f,
            Pos.x                         ,Pos.y + TexPtr->m_size.y * scale,
            0.0f                      , 1.0f
    };
...
Graphics::Sprite::Sprite(const glm::mat3x2 &Veticies, const glm::mat3x2 &TexCoords, Texture* Tex) {
    ...
    glGenBuffers(1, &EBO);
    std::vector<float> vertices{
            Veticies[0].x, Veticies[0].y,TexCoords[0].x / TexPtr->m_size.x, TexCoords[0].y / TexPtr->m_size.y,
            Veticies[1].x, Veticies[1].y,TexCoords[1].x / TexPtr->m_size.x, TexCoords[1].y / TexPtr->m_size.y,
            Veticies[2].x, Veticies[2].y,TexCoords[2].x / TexPtr->m_size.x, TexCoords[2].y / TexPtr->m_size.y
    };
    unsigned int indices[] = {
            0,1,2
    };
