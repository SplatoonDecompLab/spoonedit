...
namespace Graphics {
    ...
    FileSelectDialog::FileSelectDialog(std::string name, PathSelectType mode,
                                       std::function<void(boost::filesystem::path)> OnOpenFunc) : \
    ...
    FileSelectDialog::FileSelectDialog(std::string name, PathSelectType mode,
                                       std::function<void(boost::filesystem::path)> OnOpenFunc,
                                       std::function<void(boost::filesystem::path)> OnSaveFunc) : Widget(name, false) {
        Path = std::vector<std::string>();
        OnOpen = std::move(OnOpenFunc);
        OnSave = std::move(OnSaveFunc);
        Extension = std::vector<std::string>();
    ...
    FileSelectDialog::FileSelectDialog(std::string name, std::vector<std::string> FileType,
                                       std::function<void(boost::filesystem::path)> OnOpenFunc) :\
                                       FileSelectDialog(name,
                                                        FileType,
                                                        OnOpenFunc,
    ...
    FileSelectDialog::FileSelectDialog(std::string name, std::vector<std::string> FileType,
                                       std::function<void(boost::filesystem::path)> OnOpenFunc,
                                       std::function<void(boost::filesystem::path)> OnSaveFunc) \
: FileSelectDialog(name, File, OnOpenFunc, OnSaveFunc) {
        Extension = FileType;
    }
    ...
    void FileSelectDialog::Draw() {
        ...
        PathSelectType CurrentSelectedPathType = File;
        {
        ...
        for (boost::filesystem::directory_iterator itr(GetPath()); itr != end_iter; ++itr) {
            ...
            if (boost::filesystem::is_regular_file(itr->path())) {
                ...
                if (num == SelectedFile)
                    CurrentSelectedPathType = Folder;
            ...
            if(pathSelectMode == Folder && PathType == File) {
                ImGui::PopStyleColor();
                continue;
            }
        ...
        if (fileSelectMode == Open) {
            ...
            if (ImGui::Button("Open")) {
                if ((CurrentSelectedPathType == Folder) && pathSelectMode == File) {
        ...
        ImGui::SetCursorPosY(yPrev);
        ImGui::SetCursorPosX(
                ImGui::GetContentRegionMax().x - ImGui::CalcTextSize("Open").x - ImGui::CalcTextSize("Close").x - 30);
    ...
    REGISTERABSTRACTVCLASS(FileSelectDialog)
